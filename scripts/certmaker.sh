...
openssl x509 -req -in 2-server-issuer-csr.pem -out 2-server-issuer-ca.pem -extfile ../scripts/extensions.cnf -extensions v3_ca -CA 1-server-ca-cert.pem -CAkey 1-server-ca-key.pem -CAcreateserial -days 365
...
openssl x509 -req -in 3-server-csr.pem -out 3-server-cert.pem -CA 2-server-issuer-ca.pem -CAkey 2-server-issuer-key.pem -CAcreateserial -days 365
...
openssl x509 -req -in 5-client-issuer-csr.pem -out 5-client-issuer-ca.pem -extfile ../scripts/extensions.cnf -extensions v3_ca -CA 4-client-ca-cert.pem -CAkey 4-client-ca-key.pem -CAcreateserial -days 365
...
openssl x509 -req -in 6-application-one-csr.pem -out 6-application-one-cert.pem -CA 5-client-issuer-ca.pem -CAkey 5-client-issuer-key.pem -CAcreateserial -days 365
...
openssl x509 -req -in 7-application-two-csr.pem -out 7-application-two-cert.pem -CA 5-client-issuer-ca.pem -CAkey 5-client-issuer-key.pem -CAcreateserial -days 365
